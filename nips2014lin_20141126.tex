...
\begin{enumerate}
    ...
    \item Avoid  re-deriving inference methods for every new $\nonlin{\cdot}$.
...
\vspace{0.5cm}
%
\structure{\textbf{Main difficulty:}} The expected log likelihood cannot be computed analytically,
\begin{equation*}
...
\begin{enumerate}
  ...
  and obtain linearized objective $\tilde\Fengy$.
    \item \textbf{Newton method} on  $\tilde\Fengy$ to
        ...
        \end{equation}
       \item
       Find $\pocov$ by maximising $\tilde\Fengy$.
\end{enumerate}
%
...
\begin{table}[tb]
    ...
    %    classifiers derived \emph{specifically} for this task -- our
     ...
     Performance on USPS handwritten-digits dataset for classes `3' and `5'
     using  logistic sigmoid `forward function'.
    }
